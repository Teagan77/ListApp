...
public class SpringListServiceImplementation implements ListService {

    @Override
    public Collection add(Collection a, Object b) {

        //preventing duplicate objects from being added where applicable
        if(!a.contains(b)){
            a.add(b);
        }
        return a;
    ...
    public Collection remove(Collection a, Object b) {
        ...
        //preventing deletion of objects that do not exist in the collection
        if(a.contains(b)){
            a.remove(b);
        }else{
            System.out.println("This collection does not contain an instance of this object. Cannot remove.");
        }
        return a;
    }
